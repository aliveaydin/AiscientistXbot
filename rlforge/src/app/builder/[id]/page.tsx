...
export default function BuilderPage() {
  ...
  async function handleRename() {
    ...
    if (!trimmed || trimmed === env?.name) { setIsRenaming(false); return; }
    try {
      await updateEnvName(envId, trimmed);
      setEnv((prev: any) => ({ ...prev, name: trimmed }));
    } catch {}
    setIsRenaming(false);
  }
        ...
        <div className="flex items-center gap-1.5 text-xs min-w-0">
          ...
          {isRenaming ? (
            <input
              ...
              className="bg-[#111] border border-[#333] rounded px-1.5 py-0.5 text-white text-xs w-40 outline-none focus:border-[#555]"
            />
          ) : (
            <button onClick={() => { setEditName(env?.name || ""); setIsRenaming(true); }} className="flex items-center gap-1 group min-w-0" title="Click to rename">
              <span className="text-white font-medium truncate">{env?.name || "Loading..."}</span>
              <Pencil size={10} className="text-[#333] group-hover:text-[#888] transition-colors shrink-0" />
            </button>
          )}
...
function GitHubExportModal({ envId, env, onClose }: { envId: number; env: any; onClose: () => void }) {
  const { getToken } = useAuth();
  const [repoName, setRepoName] = useState(env?.slug || `rl-env-${envId}`);
  const [description, setDescription] = useState(env?.description?.slice(0, 100) || "RL environment generated by kualia.ai");
  ...
  async function handlePush() {
    ...
    try {
      ...
      const data = await githubExport(envId, repoName, description, isPrivate, token);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-black border border-[#1a1a1a] rounded-xl w-full max-w-lg mx-4 p-6">
        ...
        <p className="text-sm text-[#888] mb-5">
          Export environment code, training script, and README to a new GitHub repository.
        </p>

        <div className="space-y-3">
          <div>
            <label className="text-[11px] text-[#555] block mb-1">Repository name</label>
            <input value={repoName} onChange={e => setRepoName(e.target.value.replace(/[^a-zA-Z0-9_.-]/g, "-"))} className="w-full bg-[#111] border border-[#1a1a1a] rounded px-3 py-2 text-sm text-white outline-none focus:border-[#333]" />
          </div>
          <div>
            <label className="text-[11px] text-[#555] block mb-1">Description</label>
            <input value={description} onChange={e => setDescription(e.target.value)} className="w-full bg-[#111] border border-[#1a1a1a] rounded px-3 py-2 text-sm text-white outline-none focus:border-[#333]" />
          </div>
        ...
        {result?.url && (
          <div className="mt-4 p-3 bg-green-950/30 border border-green-900/50 rounded text-sm">
            <p className="text-green-400 mb-1">Repository created!</p>
        ...
        <div className="flex justify-end gap-2 mt-5">
          <button onClick={onClose} className="px-4 py-2 text-xs text-[#888] hover:text-white border border-[#1a1a1a] rounded transition-colors">Cancel</button>
          ...
          {vRuns.map(r => {
            ...
            return (
              <div key={r.id} className="border border-[#1a1a1a] rounded-lg overflow-hidden">
                <button onClick={() => r.status === "completed" ? loadReport(r.id) : setExpandedReport(isExpanded ? null : r.id)} className="w-full p-3 flex items-center gap-3 hover:bg-[#0a0a0a] transition-colors text-left">
                  <ChevronRight size={12} className={`text-[#444] shrink-0 transition-transform duration-200 ${isExpanded ? "rotate-90" : ""}`} />
                  {showCompare && r.status === "completed" && (
                    <input type="checkbox" checked={compareIds.has(r.id)} onChange={(e) => { e.stopPropagation(); toggleCompare(r.id); }} className="accent-blue-500" onClick={e => e.stopPropagation()} />
                  )}
